...
- GitHub CI, Dependabot, scoped lint, typecheck, and a test script are checked in. Twelve local tests, lint, typecheck, and the production build pass. Two early CI runs exposed a Linux lockfile mismatch and prebuild OpenNext Worker typing; both were corrected. CI passed on commit `cade60d`, including clean install, tests, typecheck, lint, build, and the critical-severity dependency audit. The four high package entries remain open with triage in [dependency-audit.md](../docs/dependency-audit.md).

...
Commit `99ec6c6` was pushed to GitHub `master`, and Cloudflare deployed the application at `https://boatship.cohortix.in`. After the CI fixes and documentation update, the Worker was redeployed as version `e3101b89-cf46-43c4-8284-a15bf138e516`. Live anonymous smoke checks returned 200 for `/`, `/login`, and `/api/auth/session` (`session: null`), and 401 for unauthenticated `/api/clients`. Authenticated staff/client workflows were not exercised because no production credentials were used. Cloudflare listed the core database, secrets master key, and realtime secrets by name without exposing their values. The existing `https://httpbin.org` webhook origin is not allowlisted, so outbound delivery to it remains blocked.

Commit `44648f8` passed GitHub CI, including clean install, tests, typecheck, lint, build, and the critical dependency gate. The same code passed a Cloudflare dry run and was deployed as Worker version `ec634ced-6d83-4e2c-b62a-ba76e1f0e2ab`. A live anonymous GET sweep of all 99 API paths returned zero 500s or failed requests; protected paths returned expected 401/403 and unsupported methods returned 405. This release includes the Hodi and product UI work already present in the working tree, along with the Neon HTTP fixes. Authenticated production mutations and external provider callbacks were not exercised.

The first GitHub CI run for `99ec6c6` failed at `npm ci`: the lockfile omitted top-level `@emnapi/core` and `@emnapi/runtime` entries required on Linux. Commit `1176932` fixed the clean install; its CI run then reached typecheck and found that `custom-worker.ts` imports an OpenNext module generated only during packaging. Commit `cade60d` added narrowly scoped TypeScript suppressions for those generated-module import lines. Its GitHub CI run passed clean install, tests, typecheck, lint, build, and the critical-severity dependency audit. The dependency metadata and type annotations do not change the deployed Worker behavior.
...
- 2026-09-25: Audited all 99 API route files for Neon HTTP transaction paths. Replaced remaining Prisma transactions, bulk mutations, and nested writes in account, Hodi, MCP, product, notification, repository, and secret flows with single statements. An anonymous GET pass found one unrelated 500 at `/api/mcp/audit`: an uncaught 401 response; the route now returns it directly. A fresh build and repeat pass over all 99 routes returned zero 500s locally and after Cloudflare deployment. Tests, typecheck, lint, build, Wrangler dry run, GitHub CI, read-only Neon SQL planning, and isolated synthetic write checks passed, with zero synthetic rows left. Authenticated production mutations remain unverified; see `api-route-audit.md`.
- 2026-09-25: Connected Hodi chat to approval-gated Boatship action proposals and added creation actions for native form templates, client form assignments, draft service templates, connected Google Calendar events, project milestones and approval requests, and product records. Each action previews and revalidates before execution; dependent requests still proceed as separate proposals. Local typecheck, lint, tests, and production build passed; authenticated end-to-end and connected Google Calendar creation were not exercised.
- 2026-09-25: Centered the product creation form within the staff content area, matching the existing client intake layout and removing the lopsided empty space on wide screens. No product data or API behavior changed.
- 2026-09-25: Corrected the Hodi chat transcript scroll effect after a streamed response triggered React's maximum update depth warning. Auto-scroll now observes stable message values and leaves scroll-position state to the scroll handler.
- 2026-09-25: Refined Hodi chat as a full-height workspace, made chat and work-queue navigation distinct, and replaced the empty queue's zero-count dashboard with a focused empty state. Chat session and queue data sources remain unchanged.
- 2026-09-25: Changed versioned snapshot writes to a single-row conditional update because Prisma `updateMany` requires a transaction unsupported by the configured Neon HTTP adapter. This fixes the local client update 500; authenticated deployment behavior has not been rechecked.
- 2026-09-25: Removed the hard-coded leadership card from the team page; staff accounts, admin badges, invites, and permissions remain as implemented.

...
- 2026-09-25: Audited all 99 API route files for Neon HTTP transaction paths. Replaced remaining Prisma transactions, bulk mutations, and nested writes in account, Hodi, MCP, product, notification, repository, and secret flows with single statements. An anonymous GET pass found one unrelated 500 at `/api/mcp/audit`: an uncaught 401 response; the route now returns it directly. A fresh build and repeat pass over all 99 routes returned zero 500s. Existing tests, typecheck, lint, production build, read-only Neon SQL planning, and isolated synthetic write checks passed, with zero synthetic rows left. Authenticated route mutations and deployment remain unverified for this change; see `api-route-audit.md`.

...
3. Verify required secrets and bindings without printing values: `DATABASE_URL`, `DIRECT_URL` for migrations, `WEBHOOK_ALLOWED_ORIGINS` for approved outbound destinations, `BOATSHIP_CRON_SECRET` for webhook retry and product reminder jobs, `BOATSHIP_SECRETS_MASTER_KEY` where user-managed provider secrets exist, and `MESSAGE_REALTIME_SECRET` where live messages are enabled. Hosted email requires `ZEPTOMAIL_API_KEY` and a verified `ZEPTOMAIL_FROM` sender, or `RESEND_API_KEY` and `RESEND_FROM`.
...
3. Apply production migrations through the controlled deploy process, deploy the exact tested build, check `GET /api/health` and non-destructive staff/client smoke flows, and monitor errors/latency, auth failures, database connectivity, webhook drain failures, and product reminder runs. Keep the preceding Worker artifact available for rollback. Verify a test webhook reaches its approved destination and the receiver deduplicates retries using `X-Boatship-Delivery`.
